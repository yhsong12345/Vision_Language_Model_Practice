...
#SBATCH --comment="Offline RL training (CQL, IQL, TD3+BC, Decision Transformer)"
...
# Parse algorithm from arguments (default: cql)
ALGO="cql"
EXTRA_ARGS=""
for arg in "$@"; do
    case $arg in
...
srun --nodes=1 --ntasks=1 -w "$head_node" \
    accelerate launch \
        ...
        --machine_rank 0 \
    train/offline_rl/train.py --algo "$ALGO" $EXTRA_ARGS &
sleep 15
